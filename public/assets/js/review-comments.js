// review-comments.js - Postar, responder e apagar comentários nas análises de jogos
...
(function () {
    ...
    const noCommentsMsg = document.getElementById('noReviewCommentsMsg');
    const commentsCount = document.getElementById('reviewCommentsCount');
    const newCommentForm = document.getElementById('newReviewCommentForm');
    const replyFormTemplate = document.getElementById('reviewReplyFormTemplate');

    function csrfToken() {
        return document.querySelector('meta[name="csrf-token"]')?.content || '';
    }

    // Mesmo estilo de toast usado no follow.js/profile-comments.js, pra não
    // depender do SweetAlert2 (que nem sempre está carregado nesta página).
    ...
    function buildCommentArticle(comment, isReply) {
        ...
        article.id = `review-comment-${comment.id}`;
    ...
    async function postComment({ content, parentId }) {
        ...
        dados.set('target_user_id', targetUserId);
        dados.set('game_id', gameId);
        dados.set('content', content);
        if (parentId) dados.set('parent_id', parentId);

        const response = await fetch('index.php?action=review_comment_add', {
            method: 'POST',
            body: dados,
        });
    ...
    commentsList.addEventListener('click', async function (event) {
        ...
        if (deleteBtn) {
            ...
            try {
                const response = await fetch('index.php?action=review_comment_delete', {
                    method: 'POST',
                    body: dados,
                });
    ...
    // Ao chegar via link com #review-comment-ID (ex: clicando numa
    // notificação), garante que o comentário-alvo fique visível: se for
    // uma resposta, o container de respostas começa fechado, então precisa
    // ser aberto antes de rolar até ele.
    function goToTargetComment() {
        const hash = window.location.hash;
        if (!hash.startsWith('#review-comment-')) return;
